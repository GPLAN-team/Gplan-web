...
export default function DrawingBoard({ size, snap, resp }: DrawingBoardProps) {
  ...
  useEffect(() => {
    const canvas = new fabric.Canvas('canvasEl', { backgroundColor: '#eee' });
  ...
  return (
    <div>
        <div className="canvasContainer">
          <canvas id="canvasEl" width={size} height={size}></canvas>
        </div>
        <Buttons>
          ...
          <Button className="citron newRectangle" onClick={() => createRectangle("#96A53A")}> +Citron </Button>
          <Button className="sand newRectangle" onClick={() => createRectangle("#D09656")}> +Sand </Button>
          <Button className="dark btn-circle" onClick={submitRFPJSON}> Make JSON </Button>
        </Buttons>
        {(resp !== null) &&
        <div>
          <Buttons>
              <Button className="blue btn" onClick={prevRFP}> Prev</Button>
              <Button className="danger btn-circle" onClick={resetDrawBoard}>Reset</Button>
              <Button className="blue btn" onClick={nextRFP}> Next </Button>
              <Button className="dark btn-circle" onClick={resetLines}> Grid On/Off </Button>
          </Buttons>
          <div>
              <h3>{indexRFP + 1} out of {listRFP.length} RFPs</h3>
          </div>
        </div>
        }
    </div>
