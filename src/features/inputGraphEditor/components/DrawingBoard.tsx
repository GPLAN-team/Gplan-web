...
export default function DrawingBoard({ size, snap }: DrawingBoardProps) {
  ...
  const drawGrid = (canvas: fabric.Canvas) => {
    ...
    for (let i = 0; i < size / snap; i++) {
      canvas.add(
        new fabric.Line([i * snap, 0, i * snap, size], {
          stroke: "#ccc",
          selectable: false,
        }),
      );
      canvas.add(
        new fabric.Line([0, i * snap, size, i * snap], {
          stroke: "#ccc",
          selectable: false,
        }),
      );
  ...
  return (
    <div>
      ...
      <div>
        <Buttons>
          <Button
            className="red newRectangle"
            onClick={() => createRectangle("#f64e60")}
          >
            {" "}
            Red{" "}
          </Button>
          <Button
            className="purple newRectangle"
            onClick={() => createRectangle("#8950fc")}
          >
            {" "}
            Purple{" "}
          </Button>
          <Button
            className="orange newRectangle"
            onClick={() => createRectangle("orange")}
          >
            {" "}
            Orange{" "}
          </Button>
          <Button
            className="dark newRectangle"
            onClick={() => createRectangle("black")}
          >
            {" "}
            Black{" "}
          </Button>
          <Button
            className="sand newRectangle"
            onClick={() => createRectangle("#D09656")}
          >
            {" "}
            Brown{" "}
          </Button>
      ...
      {resp !== null && (
        <div>
          <Buttons>
            ...
            <Button className="danger btn-circle" onClick={resetCompleteBoard}>
              Reset
            </Button>
            <Button className="blue btn" onClick={nextRFP}>
              {" "}
              Next{" "}
            </Button>
            <Button className="text-dark btn-circle" onClick={resetLines}>
              {" "}
              Grid On/Off{" "}
            </Button>
          </Buttons>
          <div>
            <h3>
              {listRFP.length > 0 ? indexRFP + 1 : 0} out of {listRFP.length}{" "}
              RFPs
            </h3>
          </div>
        </div>
